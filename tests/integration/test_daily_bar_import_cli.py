...
def test_daily_bar_import_cli_is_idempotent(tmp_path: Path) -> None:
    database_name = f"quant_core_import_cli_{uuid4().hex}"
    ...
    try:
        ...
        first_exit = import_main(["--database-url", target_url, "--input-json", str(input_path)])
        second_exit = import_main(["--database-url", target_url, "--input-json", str(input_path)])
...
def test_daily_bar_import_cli_can_feed_database_backed_paper_run(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    database_name = f"quant_core_import_sequence_{uuid4().hex}"
    target_url = f"postgresql+psycopg://quant:quant@127.0.0.1:5432/{database_name}"
    engine = _migrated_engine(database_name=database_name, target_url=target_url)
    input_path = tmp_path / "vendor_daily_bars.json"
    _write_vendor_bars(input_path)
