...
def test_persisted_pretrade_risk_gate_stores_pass_results_for_valid_state() -> None:
    database_name = f"quant_core_pretrade_risk_{uuid4().hex}"
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            decision = risk_gate.evaluate_for_strategy_run(
                session,
                strategy_run_id=persisted_strategy.run.id,
                run_mode="paper",
        ...
        assert {check.check_name for check in decision.checks} == {
            "symbol_validity",
            "gross_exposure",
