...
from quant_core.execution import OperationalStateRefresher, OrderManagementService
from quant_core.portfolio import PersistedTargetPositionBuilder
from quant_core.reconciliation import OperationalReconciliationService
...
def test_operational_state_refresher_updates_positions_and_snapshots_from_broker() -> None:
    database_name = f"quant_core_state_refresh_{uuid4().hex}"
    ...
    try:
        with Session(engine) as session:
            broker = _setup_filled_order_scenario(session)

            refresher = OperationalStateRefresher()
            refreshed = refresher.refresh_from_broker(
                session,
                run_mode="paper",
                broker=broker,
            ...
            positions = snapshot_repository.latest_positions(session, run_mode="paper")
            account = snapshot_repository.latest_account_snapshot(session, run_mode="paper")
            pnl = snapshot_repository.latest_pnl_snapshot(session, run_mode="paper")
            risk = snapshot_repository.latest_risk_snapshot(session, run_mode="paper")

        assert refreshed.position_count == 1
        assert positions[0].symbol == "SPY"
        assert positions[0].quantity == Decimal("196.850394")
        assert account is not None
        assert account.cash == Decimal("0.000000")
        assert pnl is not None
        assert pnl.total_pnl == Decimal("0.000000")
        assert risk is not None
        assert risk.gross_exposure == Decimal("1.000000")
    finally:
        _drop_database(engine=engine, database_name=database_name)


def test_operational_reconciliation_service_creates_incident_on_critical_mismatch() -> None:
    database_name = f"quant_core_reconciliation_state_{uuid4().hex}"
    target_url = f"postgresql+psycopg://quant:quant@127.0.0.1:5432/{database_name}"
    engine = _migrated_engine(database_name=database_name, target_url=target_url)
    ...
    try:
        with Session(engine) as session:
            ...
            reconciliation = OperationalReconciliationService()
            report = reconciliation.reconcile(
                session,
                run_mode="paper",
                broker=broker,
                occurred_at=datetime(2026, 4, 22, 21, 20, tzinfo=UTC),
            )
            session.commit()

            incident_repository = IncidentRepository()
            incidents = incident_repository.list_open_incidents(session, run_mode="paper")

        assert report.summary.critical_rows > 0
        assert any(
            row.category == "positions" and row.status == "quantity_mismatch" for row in report.rows
        )
        assert len(incidents) == 1
        assert incidents[0].incident_type == "reconciliation_mismatch"
    finally:
        _drop_database(engine=engine, database_name=database_name)


def _setup_filled_order_scenario(session: Session) -> FakeBrokerGateway:
